import 'package:bauxite_admin_app/customs/chauffer_widget.dart';
...
import 'package:bauxite_admin_app/pages/add_chauffer.dart';
import 'package:bauxite_admin_app/services/app_color.dart';
import 'package:bauxite_admin_app/state/user_setting_state.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';

class ChaufferPage extends StatefulWidget {
  const ChaufferPage({super.key, required this.roleId, required this.title});
  final String roleId;
  final String title;

  @override
  State<ChaufferPage> createState() => _ChaufferPageState();
}

class _ChaufferPageState extends State<ChaufferPage> {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
              onPressed: () async {
                var res = await Get.to(() => AddChaufferPage(
                    roleId: widget.roleId, title: widget.title));
                if (res == true) {
                  getData();
                }
              },
                        ...
                        return InkWell(
                          onTap: () async {
                            var res = await Get.to(() => AddChaufferPage(
                                  ...
                                  title: widget.title,
                                  chaufferModel: value[index],
                                ));
                            if (res == true) {
                              getData();
                            }
                          },
                          child: ChaufferWidget(
                              fsize: fixSize,
                              appColors: appColors,
                              chaufferModel: value[index],
                              index: index + 1),
